...
Oracle bundles a prompt and selected files into a one-shot request for Gemini. Treat Oracle output as advisory and verify it against the codebase and tests.
...
- you need a high-risk debugging, refactor, or design review from Gemini
...
- Gemini run:
  - `npx -y @steipete/oracle --engine browser --model gemini -p "<task>" --file "src/**"`
- Reattach:
...
- ask Gemini to critique the plan, identify risks, and challenge weak assumptions
...
- Prefer explicit browser mode when targeting Gemini in the normal Oracle workflow.
- Use API mode only when you intentionally want API behavior and have user consent.
...
- Treating Gemini review as a final approval gate
