---
name: oracle
description: Use when an important technical or product decision needs an external critique through Oracle CLI 0.13.0 or newer before you finalize the direction.
---
...
This skill requires Oracle CLI `0.13.0` or newer. Before using any browser-tab, harvest/live-tail, Deep Research, Project Sources, or multi-turn browser workflow, run `oracle --version`. If the installed binary is older than `0.13.0`, stop and ask the user to upgrade Oracle instead of adapting these commands to the older surface.

The `0.13.0` requirement keeps this skill aligned with the current Oracle CLI surface. This skill depends on browser workflow behavior and flags including `oracle status --browser-tabs`, `--browser-tab`, `--browser-follow-up`, `--browser-research deep`, `--browser-archive`, `--browser-attach-running`, and `oracle project-sources`.
...
- If `oracle --version` reports a version older than `0.13.0`, or if `--browser-port 55268` is unavailable, stop and report the missing prerequisite instead of retrying with a random port.
...
- Required installed version: `0.13.0` or newer.
...
- The current baseline includes renamed GPT-5.5 Pro/Thinking model-label detection, requested thinking-time handling, temporary-chat Pro selection, and MCP `consult` argument validation.
