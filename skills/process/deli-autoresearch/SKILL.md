...
1. Zero interaction — no prompting the user during a run: no Plan Mode, no question tool, no ending on a question. Continue working until the user stops you. Resolve ambiguity yourself and write the reasoning to the log (level=decision).
   - Zero interaction does not override safety gates. Get explicit user approval before external submissions, durable automations, messaging, deletion, production changes, paid resources, credential/private-data access, or any action whose tool or policy requires confirmation.
...
## 5. Usage
    ...
    /loop 2h check all tasks under <base_dir>: (1) read progress.json;
